...
boss_geo=SpatialPointsDataFrame(coords=New_coords,data=boss_geo@data[Order_all,],proj4str=CRS(laea_180_proj))
save(boss_geo,file='boss_geo_sp_tmp3.Rda')

load('boss_geo_sp_tmp3.Rda')

# formulate new SpatialPointsDataFrame where duplicated coordinates are replaced with interpolated values
# LATER - we'll need to do something more sophisticated here
#Coords = coordinates(boss_geo)
#I_dup = (Coords[2:length(boss_geo),1]==Coords[1:(length(boss_geo)-1),1] & Coords[2:length(boss_geo),2]==Coords[1:(length(boss_geo)-1),2])
#Duplicated = as.numeric(which(I_dup==1))
#Consec = which((Duplicated[2:length(Duplicated)]-Duplicated[1:(length(Duplicated)-1)])==1)
#if(length(Duplicated)>0){  #some gps values are identical - interpolate plane position in these cases
#  for(i in 1:length(Duplicated)){
#    if(Duplicated[i]<n_photos){
#      boss_geo[Which_photos[Duplicated[i]+1],]@coords = as.matrix(0.5*(boss_geo[Which_photos[Duplicated[i]],]@coords + boss_geo[Which_photos[Duplicated[i]+2],]@coords))
#      coordinates(boss_geo[Which_photos[Duplicated[i]+1],])=matrix(0.5*(boss_geo[Which_photos[Duplicated[i]],]@coords + boss_geo[Which_photos[Duplicated[i]+2],]@coords),nrow=1)
#      
#    }
#  }
#}
...
for(iid in 1:length(FlightCellSide_unique_IDs)){
  ...
  DT_diff=DT[2:length(DT)]-DT[1:(length(DT)-1)]
  Breaks = which(DT_diff>5)
  if(length(Breaks)==1){
