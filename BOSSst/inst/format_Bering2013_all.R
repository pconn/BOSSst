...
# 2013: Run analysis for 4/7 - 5/6
date.start=as.Date("2013-04-07")
date.end=as.Date("2013-05-06")
...
#remove flights outside of target altitude range
...
Obs_mat2=Obs_mat2[-which(Obs_mat2[,"species"]=="wal"),]
Obs_mat2[3715,"species"]="rn"   #discrepancies resolved in email from Erin M. 
Obs_mat2[3715,"species_conf"]="likely"
Obs_mat2[2737,"species_conf"]="guess"
...
counter=1
for(it in 1:t.steps){
  Tmp_data = Data$Grid[[it+1]]@data    #+1 hardwired in - indicates that first grid list entry is for Apr 5 but first survey occurred on Apr 6.
...
Observer_prop=as.numeric(Observer_counts[c("KYM.YANO","GAVIN.BRADY","ERIN.RICHMOND")])
...
for(iobs in 1:length(Which_replace)){ #10 such replacements
  ...
  cat(paste('replaced cell',cur_s,'time',cur_t,'with original ice_conc',Data$Grid[[cur_t+1]]@data[cur_s,"ice_conc"],'with ice=',new_ice,'\n')) #note the +1 hardwired
}
...
save(Mapping,Surveyed,Dat,K,Area_hab,Area_trans,DayHour,Thin,Prop_photo,Hab_cov,Obs_cov,Psi,file="BOSS_data_2013.Rda")
