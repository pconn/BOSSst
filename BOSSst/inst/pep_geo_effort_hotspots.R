...
strSQL <- paste("SELECT objectid,hotspotid,sealid,hotspot_found,",
                "hotspot_type,flightid,cameraid,numseals,",
                "species,species_conf,ST_AsText(geom) as wkt_geometry", 
                "FROM surveys.hotspots")
...
hotspots.df <- subset(hotspots.df,select = -c(objectid,wkt_geometry))
...
save.image(file="BOSS_Data_14Dec2015.Rdata")
