...
date.start=as.Date("2013-04-10")
date.end=as.Date("2013-05-08")
t.steps=as.numeric(date.end-date.start)
n.aero.cams=2   
n.otter.cams=3  #if 1, assumes center cam only
...
Yr13.ind=which(Year=="13")
Flights=Flt.ids[Yr13.ind]

Year=sapply(as.character(Hotspots@data[,"flightid"]),myfun)
Yr13.ind=which(Year=="13")
Hotspots=Hotspots[Yr13.ind,]


Flights=Flights[-c(30,20,28,29,32,34,36)]  # limit to 4/10-5/8
...
if(n.aero.cams==2)diameter[I_Aero==TRUE]=diameter[I_Aero==TRUE]*2
if(n.otter.cams==2)diameter[I_Aero==FALSE]=diameter[I_Aero==FALSE]+ft_to_meters*Points[["gpsalt"]][I_Aero==FALSE]*(tan(offset_otter+fov_horz/2)-tan(offset_otter-fov_horz/2))
if(n.otter.cams==3)diameter[I_Aero==FALSE]=diameter[I_Aero==FALSE]+2*ft_to_meters*Points[["gpsalt"]][I_Aero==FALSE]*(tan(offset_otter+fov_horz/2)-tan(offset_otter-fov_horz/2))
