...
#load('c:/users/paul.conn/git/BOSS/BOSS/data/AlaskaBeringData.Rdat') #duplicative
...
#analysis goes from 4th day to 32nd day; plot 4 periods (1-7,8-15,16-21,22-29) - balancing temporal spread with variable sampling effort
# sample sizes 418, 291, 347, 241 (in terms of cells surveyed)
S=length(Cur.grid)
Cur.tot=rep(0,S)
for(i in 4:10)Cur.tot=Cur.tot+Data$Grid[[i]][["ice_conc"]]
Cur.grid[["iceA"]]=Cur.tot/7
Cur.tot=rep(0,S)
for(i in 11:18)Cur.tot=Cur.tot+Data$Grid[[i]][["ice_conc"]]
Cur.grid[["iceB"]]=Cur.tot/8
Cur.tot=rep(0,S)
for(i in 19:24)Cur.tot=Cur.tot+Data$Grid[[i]][["ice_conc"]]
Cur.grid[["iceC"]]=Cur.tot/6
Cur.tot=rep(0,S)
for(i in 25:32)Cur.tot=Cur.tot+Data$Grid[[i]][["ice_conc"]]
Cur.grid[["iceD"]]=Cur.tot/8
...
tiff(file="BOSS_survey_ribbon.tiff",height=8,width=8,units='in',res=300,compression='lzw')
p
dev.off()
