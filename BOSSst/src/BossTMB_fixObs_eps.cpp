...
Type objective_function<Type>::operator() ()
{
  ...
  PARAMETER_ARRAY( Etainput_st );           // spatio-temporal random effects
  PARAMETER_VECTOR( thin_beta_day );     //extra day and day^2 effects
  PARAMETER_MATRIX(RE_tot);  //Includes both ST and iid error
  PARAMETER_VECTOR(log_sd);  //nugget (iid error on poisson intensity) sd
  ...
  for(int isp=0;isp<n_sp;isp++){
    ...
    for(int ist=0; ist<n_st; ist++){
      jnll_comp(1)-=dnorm(RE_tot(isp,ist),RE(isp,ist),exp(log_sd(isp)),true);
      Pi_s(isp,ist) = exp( linpredZ_s(ist)+ RE_tot(isp,ist) );
    }
    for(int it=0;it<n_t;it++){
      cur_sum = 0;
      for(int is=0;is<n_s;is++){
  ...
  for(int i=0;i<n_i_model;i++){
    for(int itype = 0;itype<n_obs_types;itype++){
      if( !isNA(C_i(Which_counts_model(i),itype)) ) jnll_comp(0) -= dpois( C_i(Which_counts_model(i),itype), E_count_obs(Which_counts_model(i),itype), true );
    }
  }
  ...
  //  }
  //}
  std::cout<<"logL components: "<<jnll_comp<<"\n";
