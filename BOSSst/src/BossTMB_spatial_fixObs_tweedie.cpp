...
Type objective_function<Type>::operator() ()
{
  ...
  PARAMETER_VECTOR(logit_kappa_eta);
  PARAMETER_MATRIX( Etainput_s );           // spatial random effects - (n_sp x n_eta)
  ...
  vector<Type> kappa_eta(n_sp);
  for( int isp=0; isp<n_sp; isp++){
    kappa_eta(isp)=4.0*plogis(logit_kappa_eta(isp))-4.0;
    MargSD_eta(isp) = 1 / sqrt(4.0*M_PI) / exp(logtau_eta(isp)) / kappa_eta(isp);
    Range_eta(isp) = sqrt(8.0) / kappa_eta(isp);
  }
  ...
  for(int isp=0;isp<n_sp;isp++){
    Q = Q_spde_generalized(spde, kappa_eta(isp), 2);  //default smoothing is alpha = 2
  ...
  for(int isp=0;isp<n_sp;isp++){
    for(int ieta=0;ieta<n_eta;ieta++){
      Eta_s(isp,ieta) = Etainput_s(isp,ieta) / exp(logtau_eta(isp));
    }
  }
  ...
  for(int isp=0;isp<n_sp;isp++){
    ...
    for(int is=0;is<n_s;is++){
      RE(isp,is)=0.0;
      for(int ieta=0;ieta<n_eta;ieta++){
        RE(isp,is)+=Kmap(is,ieta)*Eta_s(isp,ieta);
      }
      for(int it=0;it<n_t;it++){
        Pi_s(isp,n_s*it+is) = exp( linpredZ_s(n_s*it+is) + RE(isp,is));
      }
    }
